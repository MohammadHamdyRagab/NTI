...
const userMainHeads = [{
        name: "id",
        dataStore: null,
        default: localStorage.getItem('i'),
        isDefault: true
    },
    {
        name: "name",
...
if (addUser) {
    addUser.addEventListener("submit", function (e) {
        ...
        userMainHeads.forEach(head => {
            localStorage.setItem('i', JSON.stringify(5000));
            user.id = 5000 + (usersData.length)
            if (head.isDefault) user[head.name] = localStorage.getItem('i')
...
drawItems = () => {
    ...
    if (usersData.length == 0) {
        ...
        usersData.forEach((user, i) => {
            ...
            const traBtn = createMyOwnElement('button', td, "btn btn-success mx-3", "Transactions")
            traBtn.addEventListener('click', (e) => transactions(i))
        })
    }
}
...
transactions = (index) => {
    localStorage.setItem('transactionsIndex', index)
...
if (single) {
    try {
        ...
        showBtn.addEventListener("click", (e) => show(user))
        const traBtn = createMyOwnElement('button', td, "btn btn-success mx-3", "Transactions")
        traBtn.addEventListener('click', (e) => transactions(user))
    } catch (e) {
...
if (transaction) {
    const usersData = readDataFromStorage()
    let id = localStorage.getItem('transactionsIndex')
    let user = usersData[id]
    user.initialbalance = 'moneyamount' + (userMainHeads.initialbalance)
    userMainHeads.forEach(head => {
        transaction.elements[head.name][head.dataStore] = user[head.name]
    });
    transaction.addEventListener('submit', (e) => {
        e.preventDefault()
        userMainHeads.forEach(head => {
            if (!head.isDefault)
                usersData[id][head.name] = transaction.elements[head.name][head.dataStore]
        });
        console.log(usersData)
        setDataToStorage(usersData)
        window.location.replace("index.html")
